...
**Candy Crush Saga** es un exitoso juego móvil desarrollado por [King](https://es.wikipedia.org/wiki/King_(empresa)). Es jugado por millones de personas en todo el mundo. El juego está estructurado como una serie de niveles donde los jugadores deben unir caramelos similares para (con suerte) borrar el nivel y seguir progresando en el mapa de nivel. 
...
Ahora que ya cargamos los datos, contaremos cuántos jugadores tenemos en la muestra y cuántos días de datos tenemos. El número de jugadores es de `r length(unique(data$player_id))` y el período para el que tenemos datos es `r range(data$dt)`.
